...
class TSVLoader:
    ...
    def _load_notes(self, df: pd.DataFrame, source: Path) -> ScoreBundle:
        ...
        for _, row in df.iterrows():
            ...
            timesig = str(row.get("timesig", "")) if pd.notna(row.get("timesig")) else None
            ...
            if pd.notna(row.get("name")):
                ...
                alter = 0
                if len(name) > 1:
                    alter = name[1:].count("#") - name[1:].count("b") - name[1:].count("-")
                ...
                spc_int = int(row["tpc"]) if pd.notna(row.get("tpc")) else base_fifths.get(step, 0) + (7 * alter)
            ...
            gracenote = str(row["gracenote"]) if pd.notna(row.get("gracenote")) else None
            ...
            note_rows.append({
                ...
                # Temporal
                "quarterbeats": fraction_to_struct(qb) if qb is not None else {"num": 0, "den": 1},
                "quarterbeats_float": qb_float,
                "duration_qb": fraction_to_struct(dur_qb) if dur_qb is not None else None,
                "duration_qb_float": dur_qb_float,
                "mc": mc,
                "mn": mn,
                "mc_onset": fraction_to_struct(mc_onset) if mc_onset is not None else None,
                "mn_onset": fraction_to_struct(mn_onset) if mn_onset is not None else None,
                "timesig": timesig,
                "duration": fraction_to_struct(duration) if duration is not None else None,
                "nominal_duration": fraction_to_struct(nominal_duration) if nominal_duration is not None else None,
                "scalar": fraction_to_struct(scalar) if scalar is not None else None,
                # Pitch
