...
**Phase 6.3 redesign:** MatchClaim now has `timeline_a_id` and `timeline_b_id` as **top-level fields** (not derived from anchors). Anchors are `Optional` — only synchronous claims have them. Four case-specific constructors replace the generic `instant()`/`interval()` factories:
...
Legacy constructors `instant()` and `interval()` are kept with deprecation docstrings for backward compatibility (removal deferred to Phase 6.8).
