...
# ============================================================================
# Diagnostic Test: Print current extraction state
# ============================================================================


@pytest.mark.skipif(not specimens_available(), reason="Specimen files not found")
class TestDiagnosticOutput:
    """Diagnostic tests that print current extraction state for debugging."""

    def test_print_flow_control_comparison(self, tsv_measures, measuremap_measures):
        """Print flow control comparison across TSV and MeasureMap loaders.

        This test always passes but prints useful diagnostic info.
        Music21 is not included because we don't have MusicXML for WoO71.
        """
        loaders = {
            "TSV (Gold)": tsv_measures,
            "MeasureMap": measuremap_measures,
        }

        print("\n" + "=" * 70)
        print("FLOW CONTROL EXTRACTION COMPARISON - Beethoven WoO71")
        print("=" * 70)

        for name, measures in loaders.items():
            if measures is None:
                print(f"\n{name}: NOT AVAILABLE")
                continue

            counts = count_flow_control(measures)
            print(f"\n{name}:")
            print(f"  Total measures: {counts['total_measures']}")
            print(f"  Repeat starts: {counts['repeat_starts']}")
            print(f"  Repeat ends: {counts['repeat_ends']}")
            print(f"  Section breaks: {counts['section_breaks']}")
            print(f"  Double barlines: {counts['double_barlines']}")
            print(f"  End barlines: {counts['end_barlines']}")
            print(f"  First measure: {counts['first_measure']}")
            print(f"  Last measure: {counts['last_measure']}")
            print(f"  Volta 1 count: {counts['volta_1_count']}")
            print(f"  Volta 2 count: {counts['volta_2_count']}")
            print(f"  Has flow control: {counts['has_flow_control']}")

        print("\n" + "=" * 70)
        print("GOLD STANDARD (Expected values):")
...
# ============================================================================
# Flow Control Specimen: Diagnostic Output
# ============================================================================
...
class TestFlowControlDiagnostic:
    """Diagnostic tests that print extraction state for debugging."""

    def test_print_comparison(
        self,
        fc_tsv_measures,
        fc_measuremap_measures,
        fc_partitura_measures,
        fc_music21_measures,
    ):
        """Print flow control comparison across all loaders."""
        loaders = {
            "TSV (Gold)": fc_tsv_measures,
            "MeasureMap": fc_measuremap_measures,
            "Partitura": fc_partitura_measures,
            "Music21": fc_music21_measures,
        }

        print("\n" + "=" * 70)
        print("FLOW CONTROL COMPARISON - out_of_the_flow_experience-flow_only")
        print("=" * 70)
        ...
        print("\n" + "=" * 70)
        print("GOLD STANDARD (Expected values):")
        for key, value in FLOW_CONTROL_GOLD.items():
            print(f"  {key}: {value}")
        print("=" * 70 + "\n")
