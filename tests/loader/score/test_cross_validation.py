...
def extract_notes_df(bundle: ScoreBundle, loader_name: str) -> pd.DataFrame:
    ...
    df = df.sort_values(by=[c for c in sort_cols if c in df.columns]).reset_index(drop=True)
...
class TestNoteCountConsistency:
    ...
    def test_note_counts_exact_match(self, gold_df, partitura_df, music21_df):
        ...
        assert len(partitura_df) == 498, f"Partitura has {len(partitura_df)} notes, expected 498"
        assert len(music21_df) == 498, f"Music21 has {len(music21_df)} notes, expected 498"
...
class TestMeasureContext:
    ...
    def test_partitura_mc(self, gold_df, partitura_df):
        ...
        if mismatch_count > 0:
            ...
            explanation = MismatchExplanation.explain(
                "mc",
                gold_df["mc"].iloc[first_idx],
                partitura_df["mc"].iloc[first_idx]
            )
