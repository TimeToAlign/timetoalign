...
def profile_raw_partitura_perf(path: Path):
    print(f"--- Profiling Partitura Raw (load_performance_midi) ---")
    print(f"File: {path.name}")
    print(f"Size: {path.stat().st_size / 1024:.1f} KB")
...
def main():
    ...
    if perf_file.exists():
        t_mido = profile_loader("PerformanceMidiLoader (mido)", PerformanceMidiLoader, perf_file)
    ...
    if perf_file.exists():
        t_part_score = profile_loader("ScoreMidiLoader (partitura load_score_midi)", ScoreMidiLoader, perf_file)
