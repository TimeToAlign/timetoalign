...
from timetoalign.loader.midi import MidiEventType, PerformanceMidiLoader, ScoreMidiLoader
...
def debug_mismatch():
    ...
    # Get Note events
    mido_notes = [e for e in mido_loader.events if e["event_type"] == MidiEventType.NOTE]
    part_notes = [e for e in part_loader.events if e["event_type"] == MidiEventType.NOTE]
    
    mido_others = [e for e in mido_loader.events if e["event_type"] != MidiEventType.NOTE]
    part_others = [e for e in part_loader.events if e["event_type"] != MidiEventType.NOTE]
