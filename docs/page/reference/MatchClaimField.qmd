# MatchClaimField { #timetoalign.MatchClaimField }

```python
MatchClaimField(raw=None, *, source_fields=None, name=None, metadata=None)
```
...
| Name | Description |
| --- | --- |
| [metadata](#timetoalign.MatchClaimField.metadata) | Shared provenance applied to every materialised claim. |
| [table](#timetoalign.MatchClaimField.table) | The backing single-column :class:`pyarrow.Table`. |
| [timeline_ids](#timetoalign.MatchClaimField.timeline_ids) | The distinct timeline ids appearing in either id column. |

## Methods

| Name | Description |
| --- | --- |
| [connecting](#timetoalign.MatchClaimField.connecting) | Return the claims that involve ``timeline_id`` on either side. |
| [filter](#timetoalign.MatchClaimField.filter) | Return a filtered view following the unified-filter spirit. |
| [from_claims](#timetoalign.MatchClaimField.from_claims) | Build a field from existing :class:`MatchClaim` objects. |
| [from_columns](#timetoalign.MatchClaimField.from_columns) | Build a field directly from parallel columns (the vectorized path). |
| [from_dict](#timetoalign.MatchClaimField.from_dict) | Deserialize a field from a plain dictionary. |
| [to_claims](#timetoalign.MatchClaimField.to_claims) | Materialise every row into a list of :class:`MatchClaim` objects. |
| [to_dict](#timetoalign.MatchClaimField.to_dict) | Serialize to a plain dictionary (inverse of :meth:`from_dict`). |

### connecting { #timetoalign.MatchClaimField.connecting }

```python
MatchClaimField.connecting(timeline_id)
```
...
### filter { #timetoalign.MatchClaimField.filter }

```python
MatchClaimField.filter(timeline_id=None, timeline_ids=None)
```
...
### from_claims { #timetoalign.MatchClaimField.from_claims }

```python
MatchClaimField.from_claims(claims, *, metadata=None)
```
...
### from_columns { #timetoalign.MatchClaimField.from_columns }
...
### from_dict { #timetoalign.MatchClaimField.from_dict }

```python
MatchClaimField.from_dict(data)
```
...
### to_claims { #timetoalign.MatchClaimField.to_claims }

```python
MatchClaimField.to_claims()
```
...
### to_dict { #timetoalign.MatchClaimField.to_dict }

```python
MatchClaimField.to_dict()
```
