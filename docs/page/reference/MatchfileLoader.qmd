...
|---------------------|-----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------|
| score_unit          | [TimeUnit](`timetoalign.core.TimeUnit`) | Primary unit for the score timeline. Default ``TimeUnit.quarters``.                                                                                                                                                | `TimeUnit.quarters` |
| normalize_anacrusis | [bool](`bool`)                          | If True (default), attach a ``ShiftMap`` that maps raw coordinates to normalised (non-negative) coordinates. Has no effect on the stored event coordinates, which are always the normalised (non-negative) values. | `True`              |
...
| Name           | Type                 | Description                                                                                                                                                                                                                                                                                                   | Default   |
|----------------|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| score_timeline | \'Timeline \| None\' | Optional pre-existing score timeline. When supplied, the bundle uses this timeline instead of the loader's internally built one. MatchClaims are NOT rebound — they still reference the internally built score timeline's uid. For correct cross-referencing, the external timeline should have the same uid. | `None`    |
| verify         | [bool](`bool`)       | If ``True`` (default) and *score_timeline* is not ``None``, verify that internally built events are compatible with the external timeline. Set to ``False`` to skip verification (e.g. when the external timeline is known to be authoritative and you accept any coordinate differences).                    | `True`    |

#### Returns {.doc-section .doc-section-returns}

| Name   | Type                | Description                                                    |
|--------|---------------------|----------------------------------------------------------------|
|        | \'AlignmentBundle\' | ``AlignmentBundle`` with score group + standalone performances |
|        | \'AlignmentBundle\' | + MatchClaims.                                                 |
...
|        | [ValueError](`ValueError`)     | If *verify* is ``True`` and the external timeline has incompatible event coordinates. |
...
| Name   | Type         | Description                                                                                                 | Default    |
|--------|--------------|-------------------------------------------------------------------------------------------------------------|------------|
| uid    | [str](`str`) | Timeline uid (e.g. ``"score:clt1"``), role shorthand (``"score"``, ``"perf:1"``), or partial/regex pattern. | _required_ |

#### Returns {.doc-section .doc-section-returns}

| Name   | Type         | Description            |
|--------|--------------|------------------------|
|        | \'Timeline\' | The matching Timeline. |

#### Raises {.doc-section .doc-section-raises}

| Name   | Type                           | Description                                       |
|--------|--------------------------------|---------------------------------------------------|
|        | [KeyError](`KeyError`)         | If no timeline with the given uid or role exists. |
|        | [RuntimeError](`RuntimeError`) | If ``load()`` has not been called yet.            |
...
| Name   | Type                           | Description                                                |
|--------|--------------------------------|------------------------------------------------------------|
|        | [list](`list`)\[\'Timeline\'\] | List of Timeline objects. Empty if ``load()`` has not been |
|        | [list](`list`)\[\'Timeline\'\] | called yet.                                                |
...
| Name     | Type                                   | Description                | Default   |
|----------|----------------------------------------|----------------------------|-----------|
| *sources | [Path](`pathlib.Path`) \| [str](`str`) | Paths to ``.match`` files. | `()`      |
...
| Name   | Type                                     | Description                             |
|--------|------------------------------------------|-----------------------------------------|
|        | [FileNotFoundError](`FileNotFoundError`) | If any source path does not exist.      |
|        | [ValueError](`ValueError`)               | If any source is not a ``.match`` file. |
